...
*/
bool MPR121::begin(uint8_t num_electrode) {
  ...
  setThresholds(_touchThres, _releaseThres);
  ...
  byte ECR_SETTING =
    B10000000 + num_electrode; // 5 bits for baseline tracking & proximity disabled + X
...
*/
void MPR121::setThresholds(uint8_t touch_thres, uint8_t release_thres) {
  _touchThres = touch_thres;
  _releaseThres = release_thres;
  // set all thresholds (the same)
  for (uint8_t i = 0; i < 12; i++) {
    writeRegister(MPR121_TOUCHTH_0 + 2 * i, _touchThres);
    writeRegister(MPR121_RELEASETH_0 + 2 * i, _releaseThres);
  }
}
...
*/
uint8_t MPR121::touched_state_8() {
  Wire.requestFrom(_addr, (uint8_t)1);
  return Wire.read();
}
uint16_t MPR121::touched_state_12() {
  Wire.requestFrom(_addr, (uint8_t)2);

  LSB = Wire.read();
  MSB = Wire.read();

  uint16_t touch_state = ((MSB << 8) | LSB); //16bits that make up the touch states
...
uint8_t MPR121::readRegister8(uint8_t reg) {
  ...
  Wire.beginTransmission(_addr);    // Get the slave's attention, tell it we're sending a command byte
  Wire.write(reg);                               //  The command byte, sets pointer to register with address of 0x32
  Wire.requestFrom(_addr, (uint8_t)1);         // Tell slave we need to read 1byte from the current register
  uint8_t slaveByte = Wire.read();        // read that byte into 'slaveByte2' variable
...
uint16_t MPR121::readRegister16(uint8_t reg) {
  Wire.beginTransmission(_addr);    // Get the slave's attention, tell it we're sending a command byte
  Wire.write(reg);                               //  The command byte, sets pointer to register with address of 0x32
  Wire.requestFrom(_addr, (uint8_t)2);         // Tell slave we need to read 1byte from the current register
  byte LSB = Wire.read();
  byte MSB = Wire.read();
  uint16_t slaveByte = ((MSB << 8) | LSB); //16bits that make up the touch states
  Wire.endTransmission();                  // "Hang up the line" so others can use it (can have multiple slaves & masters connected)
