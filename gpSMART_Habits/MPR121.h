...
class MPR121 {
  ...
  public:
    // Hardware I2C
    MPR121(uint8_t i2caddr = MPR121_I2CADDR_DEFAULT);

    bool begin(uint8_t num_electrode);
    ...
    uint8_t touched_state_8();
    uint16_t touched_state_12();
    void setThresholds(uint8_t touch_thres, uint8_t release_thres);

  private:
    uint8_t _addr = MPR121_I2CADDR_DEFAULT;
    uint8_t _touchThres = MPR121_TOUCH_THRESHOLD_DEFAULT;
    uint8_t _releaseThres = MPR121_RELEASE_THRESHOLD_DEFAULT;
    uint8_t LSB;
    uint8_t MSB;
